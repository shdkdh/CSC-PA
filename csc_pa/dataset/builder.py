...
from .BUS import build_busi_semi_loader, build_busi_loader
...
def get_loader(cfg, seed=0):
    ...
    elif cfg_dataset["type"] == "BUSI_semi":
        train_loader_sup, train_loader_unsup = build_busi_semi_loader(
            "train", cfg, seed=seed
        )
        val_loader = build_busi_loader("val", cfg)
        logger.info("Get loader Done...")
        return train_loader_sup, train_loader_unsup, val_loader

    elif cfg_dataset["type"] == "BUSI":
        train_loader_sup = build_busi_loader("train", cfg, seed=seed)
        val_loader = build_busi_loader("val", cfg)
        logger.info("Get loader Done...")
        return train_loader_sup, val_loader
