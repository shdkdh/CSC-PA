...
class busi_dset(BaseDataset):
    def __init__(
            self, data_root, data_list, trs_form, trs_form_strong=None,
            seed=0, n_sup=258, split="val", flag_semi=False,
            mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
    ):
        # super(busi_dset, self).__init__(data_list)
        ...
        if self.split == 'train':
            ...
            with open(self.data_list, 'r') as f:
                self.img_list = f.readlines()
            f.close()
        self.img_list = [item.split('\t')[0].split('/')[1] for item in self.img_list]
        # oversamplying labeled data for semi-supervised training
        if len(self.img_list) >= n_sup and split == "train":
            self.list_sample_new = random.sample(self.img_list, n_sup)
        elif len(self.img_list) < n_sup and split == "train":
            num_repeat = math.ceil(n_sup / len(self.img_list))
            self.img_list = self.img_list * num_repeat
            self.list_sample_new = random.sample(self.img_list, n_sup)
        else:
            self.list_sample_new = self.img_list
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join(self.data_root, 'img', self.list_sample_new[index])
        if 'UDIAT' in self.data_root:
            label_path = os.path.join(self.data_root, 'label', self.list_sample_new[index])
        else:
            label_path = os.path.join(self.data_root, 'label', self.list_sample_new[index][:-4] + '_mask.png')
        ...
        if self.transform_strong is None:
            ...
            if not self.flag_semi:
                return self.list_sample_new[index], image, label
            else:
                return self.list_sample_new[index], image, image.clone(), label
        else:
            # apply augmentation
            ...
            return self.list_sample_new[index], image_weak, image_strong, label
...
def build_busi_loader(split, all_cfg, seed=0):
    ...
    # n_sup = cfg.get("n_sup", 1)
    if 'UDIAT' in cfg["data_root"]:
        n_sup = cfg.get("n_sup", 132)
    else:
        n_sup = cfg.get("n_sup", 517)
    ...
    # create dataset
    dset = busi_dset(cfg["data_root"], cfg["data_list"], trs_form, None,
...
def build_busi_semi_loader(split, all_cfg, seed=0):
    split = "train"
    ...
    # n_sup = cfg.get("n_sup", 1) # oversample labeled data to the amount of unlabeled data
    if 'UDIAT' in cfg["data_root"]:
        n_sup = 132 - cfg.get("n_sup", 132)
    else:
        n_sup = 517 - cfg.get("n_sup", 517)
    ...
    dset = busi_dset(cfg["data_root"], cfg["data_list"], trs_form_weak, None,
    ...
    dset_unsup = busi_dset(cfg["data_root"], data_list_unsup, trs_form_weak, trs_form_strong,
                           seed, n_sup, split,
                           flag_semi=True,
                           mean=mean, std=std)
