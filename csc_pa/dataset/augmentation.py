...
def weak_labeled_data(data, target):
    batch_size, _, im_h, im_w = data.shape
    device = data.device
    ...
    for i in range(batch_size):
        if mode == "flip":
            transform = transforms.Compose([
                transforms.RandomHorizontalFlip(p=0.5),  # 指定是否翻转
            ])
            img_w = transform(data[i])
            target_w = transform(target[i])

        if mode == "crop":
            p = random.random()
            p = p if p <= 0.5 else p-0.5
            transform = transforms.Compose([
                transforms.RandomCrop(size=im_h, padding=int(im_h * p), padding_mode='reflect')
            ])
            img_w = transform(data[i])
            target_w = transform(target[i])

        new_data.append(img_w.unsqueeze(0))
        new_target.append(target_w.unsqueeze(0))
...
def weak_unlabeled_data(data, target, mode="cutout"):
    batch_size, _, im_h, im_w = data.shape
    device = data.device
